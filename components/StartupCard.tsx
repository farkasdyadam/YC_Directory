...
export default function StartupCard({post}: { post: StartupTypeCard }) {
    const {_createdAt, author: {_id: authorId, name}, views, title, category, _id, description} = post;
    return (
        <li className={"startup-card group"}>
            ...
            <div className={"flex justify-between mt-5 gap-5"}>
                <div className={"flex-1/2"}>
                    <Link href={`/user/${authorId}`}>
                        <p className={"text-sm/[16px] font-bold line-clamp-1 mb-2"}>{name}</p>
                    </Link>
            ...
            <Link href={`/startup/${_id}`}>
                ...
                <img src={"https://placehold.co/100x100"} alt={"placeholder"} className={"startup-card_img"}/>
            </Link>
